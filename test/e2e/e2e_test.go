...
const namespace = "dragonfly-system"
...
var _ = Describe("Manager", Ordered, func() {
	...
	BeforeAll(func() {
		...
		cmd := exec.Command("sh", "-c",
			fmt.Sprintf("kubectl create ns %s --dry-run=client -o yaml | kubectl apply -f -", namespace))
		_, err := utils.Run(cmd)
...
func verifyInjection(podName, namespace string) func(g Gomega) {
	return func(g Gomega) {
		...
		g.Expect(podJson).To(ContainSubstring(injector.BinaryVolumeName),
			"Should have binary volume")
		g.Expect(podJson).To(ContainSubstring(injector.InitContainerName),
			"Should have binary init container")
	}
}
